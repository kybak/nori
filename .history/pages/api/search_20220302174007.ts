...
const resolvers = {
  Query: {
    async search(parent, args, context) {
      ...
      let amountFilled: number = 0; // 0
      let amountNeeded: number = args.quantity; // 10

      for (let item of itemsData) {
        let remaining: number = amountNeeded - amountFilled; // 10

        if (item.quantity <= remaining) {
          amountFilled += item.quantity;
        }

        // if diff greater than available qty, take qty from diff
        if (remaining > item.quantity) {
          remaining -= item.quantity;
        }

        let amountToTake: number = remaining;

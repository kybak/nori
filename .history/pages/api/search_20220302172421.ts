  ...
  Query: {
    async search(parent, args, context) {
      ...
      for (let item of itemsData) {
        // get diff between amount needed and current amount available
        // if there is still some amount needed take that from the next amount available

        const diff: number = amountNeeded - item.quantity;

        if (diff > 0) {
          // set amount needed to diff
          amountNeeded = diff;
        }
